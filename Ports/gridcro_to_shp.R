...
inputDir <- "E:/Research/Ports/"

nc <- nc_open("E:/Research/Ports/GRIDCRO2D.nc")   #opening in ncdf to check it out

# Grab the lat and lon from the data
lat <- raster("E:/Research/Ports/GRIDCRO2D.nc", varname="LAT")
meanlat <- cellStats(lat, mean)
lon <- raster("E:/Research/Ports/GRIDCRO2D.nc", varname="LON")
...
ras <- raster("E:/Research/Ports/GRIDCRO2D.nc", varname = "HT")   #opening with random variable using the raster package
...
#SAVING 
writeOGR(obj=shape, dsn="E:/Research/Ports/shapefiles/ports_shp.shp", layer = "ports_shp",  driver="ESRI Shapefile")
