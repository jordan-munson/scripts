...
for t in range(0, date_diff):
    # read combined 
    modeloutput= "/data/airpact5/saved/"+now.strftime("%Y")+ "/"+now.strftime("%m")+"/aconc/combined_" +  now.strftime("%Y%m%d") +".ncf"
    print(modeloutput)
...
writer = pd.ExcelWriter('AIRPACT4km_at_Urbanova_ref_site.xlsx')
