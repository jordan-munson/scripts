...
modeloutputs.append(r'E:\Research\Urbanova_Jordan\Urbanova_ref_site_comparison/AIRPACT/' + now.strftime('%Y')+'/'+now.strftime('%Y%m%d')+'00'+'/POST/CCTM/combined_' + now.strftime('%Y%m%d')+'.ncf')
print(modeloutputs)
...
for t in range(0, date_diff):
    ...
    if os.path.isfile(r'E:\Research\Urbanova_Jordan\Urbanova_ref_site_comparison/AIRPACT/' + now.strftime('%Y')+'/'+now.strftime('%Y%m%d')+'00'+'/POST/CCTM/combined_' + now.strftime('%Y%m%d')+'.ncf'):
...
m = Basemap(projection='lcc', width=w, height=h, lat_0=lat_0, lon_0=lon_0,
              llcrnrlon = lon[186,136], urcrnrlon = lon[215-1,165-1], 
              llcrnrlat = lat[186,136], urcrnrlat = lat[215-1,165-1], resolution='h', area_thresh=1000)# setting area_thresh doesn't plot lakes/coastlines smaller than threshold
x,y = m(lon,lat)
var_list = ["O3", "PMIJ"]
...
############################################
#save maps into the pdf file (two maps in single page)
with PdfPages(base_dir+'maps/airpact_avg_basemap_' + '_'+ start.strftime("%Y%m%d") + '-' +  end.strftime("%Y%m%d") + '.pdf') as pdf:
