...
met_path = r'E:\Research\Urbanova_Jordan\Urbanova_ref_site_comparison\AIRPACT/'
...
modeloutputs = []
modeloutputs.append(met_path + now.strftime('%Y')+'/'+now.strftime('%Y%m%d')+'00/MCIP37/METCRO2D')
print(modeloutputs)
now += timedelta(hours=24)
# For loop to find the combined3D files for specified time period
for t in range(0, date_diff):
...
airpact_met = met_path + start.strftime('%Y')+'/'+start.strftime('%Y%m%d')+'00/MCIP37/METCRO2D'
print(airpact_met)
fin0 = Dataset(airpact_met, 'r')
TEMP2 = fin0.variables['TEMP2'][0,0]
TEMPG = fin0.variables['TEMPG'][0,0]
w = (fin0.NCOLS)*(fin0.XCELL)
h = (fin0.NROWS)*(fin0.YCELL)
lat_0 = fin0.YCENT
lon_0 = fin0.XCENT
...
#airpact_met = get_airpact_DF(start, end, layer)
#print(airpact_met.keys())
#print(airpact_met['PMIJ'].shape)
#print(airpact_met['DateTime'].shape)
#print(airpact_met['lat'].shape)

airpact_met['lat'] = airpact_met['lat'][:,186:216,136:166]
airpact_met['lon'] = airpact_met['lon'][:,186:216,136:166]
airpact_met['TEMP2'] = airpact_met['TEMP2'][:,186:216,136:166]
airpact_met['TEMPG'] = airpact_met['TEMPG'][:,186:216,136:166]
airpact_met['DateTime'] = airpact_met['DateTime'][:,186:216,136:166]
...
airpact['PMIJ'] =  airpact_3d['PMIJ']/airpact_2d['PMIJ']
############################################
# Ratio averaged domain basemaps          Positive values represent an overestimation from 1p33km. The maps are 1p33km/4km
############################################
#save maps into the pdf file (two maps in single page)
...
#Met map
############################################
# Met averaged domain basemaps         
############################################
#save maps into the pdf file (two maps in single page)
var_list = ["TEMP2", "TEMPG"]
with PdfPages(base_dir+'maps/temp_avg_basemap_' + '_'+ start.strftime("%Y%m%d") + '-' +  end.strftime("%Y%m%d") + '.pdf') as pdf:
    ...
    for i, sp in enumerate(var_list):
        ...
        # compute auto color-scale using maximum concentrations
        down_scale = np.percentile(airpact_met[sp], 5)
        up_scale = np.percentile(airpact_met[sp], 95)
        clevs = np.round(np.arange(down_scale, up_scale, (up_scale-down_scale)/10),3)
        print("debug clevs", clevs, sp)
        
        cblabel = unit_list[i]
        cbticks = True
        cs = m.contourf(x,y,airpact_met[sp].mean(axis=0),clevs,cmap=plt.get_cmap('jet'), extend='both')
        cs.cmap.set_under('cyan')
        cs.cmap.set_over('black')
...
#        m.drawcounties()
        ...
        # print the surface-layer mean on the map plot
        plt.annotate("mean: " + str(airpact_met[sp].mean(axis=0).mean()) +" ppb", xy=(0, 1.02), xycoords='axes fraction')
