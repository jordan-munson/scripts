...
versions = ['AP-3','AP-4','AP-5']#,'difference'] #List versions
...
for species in pollutant:
    ...
    for version in versions:
        ...
        m.drawstates()
        m.drawcountries()
        cmap = plt.get_cmap('seismic') # jet
        if version == 'difference':
            
            used_AQSID = list(set(df_mod3['AQSID']))
            #used_AQSID = ['530330010','530090013'] # to test map
...
# =============================================================================
                #This section selects only data relevant to the aqs site
                d = df_mod3.loc[df_mod3['AQSID'].astype(str)==AQSID]
                d1 = df_mod4.loc[df_mod4['AQSID'].astype(str)==AQSID]
                d1 = d1.reset_index()
                d=d.reset_index()
    #            site_nameinfo = d.loc[0,'Local Site Name'] #Gets the longname of the site to title the plot
...
# =============================================================================
#                 except KeyError:
#                     print('Empty dataframe for '+AQSID)
#                     continue
# =============================================================================
                #lats.append(lat)
                #lons.append(lon)
                ...
                if species=='O3':
                    ...
                    x = d1.resample('H').mean()
                    avg_8hr_o3 = x.rolling(8,min_periods=6).mean()
                    times = avg_8hr_o3.index.values - pd.Timedelta('8h')
                    avg_8hr_o3.index.values[:] = times
                    d1 = avg_8hr_o3.resample('D').max().dropna()
                    sze_scale = 1.7 # 0.7
                ...
                if species=='PM2.5':
                    ...
                    d1 = d1.resample('D').mean() # resample to 24 hour average
                    sze_scale = 0.5 # 0.7
            
            
                #site_type = d.loc[0,'Location Setting']
                if species == 'O3_hourly' or species == 'PM2.5_hourly':
                ...
                try:
                    ...
                    if species == 'O3_hourly':
                        plotted_figure.set_clim(-50,50) # -50, 50
                    else:
                        plotted_figure.set_clim(-100,100) # -100, 100
                    #print(aq_stats[species+'_mod'][sp])
                # aq_stats.columns = aq_stats.columns.str.replace(abrv+'_AP5_4km', '4km ' + site_nameinfo)     
            ...
            used_AQSID = list(set(df_mod1['AQSID']))
            #used_AQSID = ['530630001']#,'530090013'] # to test map
            df_mod2 = df_mod1.copy()
            for AQSID in used_AQSID:
# =============================================================================
#                 try:
# =============================================================================
                #This section selects only data relevant to the aqs site
                d = df_mod2.loc[df_mod2['AQSID'].astype(str)==AQSID]
                d=d.reset_index()
    #            site_nameinfo = d.loc[0,'Local Site Name'] #Gets the longname of the site to title the plot
                
                lat = d.loc[0,'Latitude']
                lon = d.loc[0,'Longitude']
                sitetype = d.loc[0,'Location Setting']
                sitename = d.loc[0,'Local Site Name']
                #print(AQSID)
# =============================================================================
#                 except KeyError:
#                     print('Empty dataframe for '+str(AQSID))
#                     continue
# =============================================================================
                #lats.append(lat)
                #lons.append(lon)
                ...
                if species=='O3':
                    var_name_mod1 = 'O3_mod'
                    var_units = 'ppb'
                    d = d.set_index('datetime').drop(['PM2.5_obs','PM2.5_mod'],axis=1)
                    x = d.resample('H').mean()
                    avg_8hr_o3 = x.rolling(8,min_periods=6).mean()
                    times = avg_8hr_o3.index.values - pd.Timedelta('8h')
                    avg_8hr_o3.index.values[:] = times
                ...
                if species=='PM2.5':
                    var_name_mod1 = 'PM2.5_mod'            
                    var_units = 'ug/m3'
                    d = d.set_index('datetime').drop(['O3_obs','O3_mod'],axis=1)
                    d = d.resample('D').mean() # resample to 24 hour average
                    sze_scale = .5 # 0.7
                ...
                # aq_stats.columns = aq_stats.columns.str.replace(abrv+'_AP5_4km', '4km ' + site_nameinfo)     
                    ...
                    # Merge stats into single dataframe
                    aq_stats.columns = aq_stats.columns.str.replace(species+'_mod', species+' ' + ' '+version+' '+AQSID)    
                    stats_com = pd.merge(stats_com, aq_stats, how = 'inner', left_index = True, right_index = True)     
                    
                    stats1 = stats_version(d, var_name_mod1, var_name) # run this so other scripts dont hit errors
                    ...
                    stats_T['AQSID'] = AQSID
                    stats_T['Latitude'] = str(lat)
                    stats_T['Longitude'] = str(lon)
                    stats_T['SiteType'] = sitetype
                    stats_T['SiteName'] = sitename
...
# =============================================================================
        ...
        if abc == '(c)':
            if species == 'O3_hourly' or species == 'O3':
                ...
                # Circle size chart
                msizes = [0,25,50,75,100,125]
                msizes=[i * sze_scale for i in msizes]
                labels = ['FE [%]',25,50,75,100,125]
            markers = []
            for size,label in zip(msizes,labels):
                markers.append(plt.scatter([],[], s=size, label=label,c='black'))
    ...
    plt.savefig(outputDir+'/plots/bias_maps/'+species+'_bias_map.png',  pad_inches=0.1, bbox_inches='tight')
    plt.show()
    plt.close()
    
#stats_com = stats_com.T
stats_all = stats_all.reset_index()        
stats_all.to_csv(outputDir+'/stats/aqs_location_stats.csv')
...
# 
# # =============================================================================
# # This method will make a difference plot
# # =============================================================================
# 
# # create lits to use in calcs of site ids
# used_AQSID = list(set(df_com['AQSID']))
...
# #pollutant = ['O3_hourly','PM2.5_hourly']
# 
# versions = ['difference'] #List versions
# df_com['AQSID'] = df_com['AQSID'].astype(str) # For some reason some of these are int64, this makes everything a string
# 
# for species in pollutant:
...
#                     var_name_mod1 = 'O3_mod'
#                     var_units = 'ppb'
#                     d = d.set_index('datetime').drop(['PM2.5_mod','PM2.5_obs'],axis=1)
#                     d1 = d1.set_index('datetime').drop(['PM2.5_mod','PM2.5_obs'],axis=1)
#                     
#                     x = d.resample('H').mean()
#                     avg_8hr_o3 = x.rolling(8,min_periods=6).mean()
#                     times = avg_8hr_o3.index.values - pd.Timedelta('8h')
...
#                     var_name_mod1 = 'PM2.5_mod'            
#                     var_units = 'ug/m3'
#                     d = d.set_index('datetime').drop(['O3_mod','O3_obs'],axis=1)
#                     d1 = d1.set_index('datetime').drop(['O3_mod','O3_obs'],axis=1)
#                     d = d.resample('D').mean().dropna() # resample to 24 hour average
#                     d1 = d1.resample('D').mean()#.dropna() # resample to 24 hour average
#                     sze_scale = 3
# 
#             
#             
...
#                     else:
#                         plotted_figure.set_clim(-50,50) # -100, 100
#                     #print(aq_stats[species+'_mod'][sp])
#                 # aq_stats.columns = aq_stats.columns.str.replace(abrv+'_AP5_4km', '4km ' + site_nameinfo)     
#            
#                     # Merge stats into single dataframe
...
# # =============================================================================
# #         if species == 'O3_hourly' or species == 'O3':
# #             msizes = [0,3,6,9,12,15]
# #             msizes=[i * sze_scale for i in msizes]
# #             labels = ['FE [%]',3,6,9,12,15]
# #         else:   
# #             # Circle size chart
# #             msizes = [0,10,20,30,40,50]
# #             msizes=[i * sze_scale for i in msizes]
...
# #         for size,label in zip(msizes,labels):
# #             markers.append(plt.scatter([],[], s=size, label=label,c='black'))
# #         plt.legend(bbox_to_anchor=(1.08, 1), loc='upper left',handles=markers,labelspacing=1) 
# # =============================================================================
#             
#     # overall colorbar  
...
#     
#     plt.savefig(outputDir+'/plots/bias_maps/'+species+'_bias_map_difference.png',  pad_inches=0.1, bbox_inches='tight')
#     plt.show()
#     plt.close()
# =============================================================================
