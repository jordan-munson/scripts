...
# df10 = pd.read_csv(base_dir + '/airpact_met_data/AQS/airpact_aqs_met_2018815_20181231.csv').drop(['Unnamed: 0','ix','iy'],axis=1)
# print('Met data read')
# 
...
setting =['total']
versions = ['AP-3','AP-4','AP-5'] #List versions
...
#         stats1 = met.stats(df_all, var_name_mod1, var_name, var_units)
# =============================================================================
        df_all = df_all.dropna(subset = [var_name_mod1])
        stats1 = stats_version(df_all, var_name_mod1, var_name)
        stats1.loc['model'] = ['AIRPACT']
        ...
        ax.text(1.15, 0.4,'$r^2$ = %s' %stats_T['R^2 [-]'][0], 
                 ha='center', va='center', transform=ax.transAxes)
        ax.text(1.15, 0.3,'RMSE= %s' %stats_T['RMSE'][0], 
                 ha='center', va='center', transform=ax.transAxes)
...
for w in new_list:
   ...
   # if var_name=='precip_accum':
    ...
    stats1 = met.stats(df_all.dropna(), var_name_mod1, var_name, var_units)
    stats1.loc['model'] = ['AIRPACT']
    stats_combined = pd.concat([stats1],axis=1,join_axes=[stats1.index])
...
#         # UW WRF model output        
#         ax.plot(df_all.index,np.cos(df_all[var_name_mod1]*np.pi/180),c='r',label='Model')
#         ylabel='Cosine of Wind Direction'
# =============================================================================
        ax.plot(df_all.index,df_all[var_name],c='k',label='Observations')
        # UW WRF model output        
        ax.plot(df_all.index,df_all[var_name_mod1],c='r',label='Model')
        ...
        ax.set(title=st_name,xlabel='Date / Time (UTC)',ylabel=ylabel,ylim=(ymin,ymax))
    else:
        ax.plot(df_all.index,df_all[var_name],c='k',label='Observations')
        # UW WRF model output        
        ax.plot(df_all.index,df_all[var_name_mod1],c='r',label='Model')
