...
grid_dir_4km='/data/lar/projects/airpact5/AIRRUN/2018/2018011100/save_MCIP37/'
urb_path = '/data/lar/projects/Urbanova/'
airpact_path = '/data/lar/projects/airpact5/saved/'
...
for t in range(0, date_diff):
    ...
    if os.path.isfile(airpact_path + now.strftime('%Y')+'/'+ now.strftime('%m')+'/aconc'+'/combined_' + now.strftime('%Y%m%d')+'.ncf'):
...
name = '4km_'+start.strftime("%Y%m%d")+'_'+end.strftime("%Y%m%d")
