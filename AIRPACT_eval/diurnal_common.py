...
# load data
df_com = pd.read_csv(inputDir+'AQS_data/df_com_aplong_pst.csv').drop('Unnamed: 0', axis=1)
...
#         data_obs = []
#         data_mod = []
#         time_obs = [time_0_obs,time_1_obs,time_2_obs,time_3_obs,time_4_obs,time_5_obs,time_6_obs,time_7_obs,time_8_obs
...
#         time_mod = [time_0_mod,time_1_mod,time_2_mod,time_3_mod,time_4_mod,time_5_mod,time_6_mod,time_7_mod,time_8_mod
...
#         for time_data in time_obs:
#             data_obs.append(time_data)
# 
#         for time_data in time_mod:
#             data_mod.append(time_data)
#             
#         for time_data_obs, time_data_mod in zip(time_obs,time_mod):
#             stats_obs = pd.DataFrame(time_data_obs)
#             stats_mod = pd.DataFrame(time_data_mod)
#             
...
for species in pollutant:
    fig = plt.figure(dpi=300,figsize= (6.125,7)) #This is as small as can currently go without having some overlap of labels 14,16 for a presentation size plot
...
# =============================================================================
    ...
    fig.tight_layout() # spaces the plots out a bit
    if species == 'O3':
        plt.savefig(inputDir+'/plots/diurnal/'+'O3_diurnal_common.png',  pad_inches=0.1, bbox_inches='tight')
    else:
        plt.savefig(inputDir+'/plots/diurnal/'+'PM_diurnal_common.png',  pad_inches=0.1, bbox_inches='tight')
    plt.show()
    plt.close()        
...
# =============================================================================
# Seasonal diurnal plots both seasons combined into 1 plot
# =============================================================================
...
versions = ['AP-3','AP-4','AP-5']
seasons = ['Summer','Winter']
hourly_table = pd.DataFrame(columns = ['O3_obs', 'PM2.5_obs', 'O3_mod', 'PM2.5_mod'])
for season in seasons:
    ...
    # =============================================================================
            ...
            table['Season'] = season
            table['Metric'] = 'Hourly Mean'
            table['Unit'] = unit_list
            hourly_table = pd.concat([hourly_table,table])
    ...
    fig.tight_layout() # spaces the plots out a bit
    if species == 'O3':
        plt.savefig(inputDir+'/plots/diurnal/'+'O3_diurnal_'+season+'_common.png',  pad_inches=0.1, bbox_inches='tight')
    else:
        plt.savefig(inputDir+'/plots/diurnal/'+'PM_diurnal_'+season+'_common.png',  pad_inches=0.1, bbox_inches='tight')
    plt.show()
    plt.close()       
    print('The plot above is ' + season)
hourly_table = hourly_table.rename(columns={'O3_mod':'O3_Forecast','O3_obs':'O3_Observation'}).drop(['PM2.5_mod','PM2.5_obs'],axis=1)
hourly_table.to_csv(inputDir+'/stats/diurnal_hourly_mean.csv')
#%%
...
versions = ['AP-3','AP-4','AP-5']
seasons = ['Summer','Winter']
fig = plt.figure(dpi=100,figsize= (8,7)) #This is as small as can currently go without having some overlap of labels 14,16 for a presentation size plot
for season in seasons:
    for species in pollutant:
        ...
        for version,i,j in zip(versions,[1,3,5],[2,4,6]):
            if version == 'AP3':
                xlabel = 'AP-3'
                abc = '(a)'
...
fig.tight_layout() # spaces the plots out a bit
if species == 'O3':
    plt.savefig(inputDir+'/plots/diurnal/'+'O3_diurnal_summer_winter_common.png',  pad_inches=0.1, bbox_inches='tight')
else:
    plt.savefig(inputDir+'/plots/diurnal/'+'PM_diurnal_summer_winter_common.png',  pad_inches=0.1, bbox_inches='tight')
plt.show()
plt.close()       
