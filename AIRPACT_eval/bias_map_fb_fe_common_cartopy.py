...
# =============================================================================
    fig, ax = plt.subplots(1,3,subplot_kw={'projection': ccrs.Mercator()}, dpi=300)
...
# =============================================================================
        ...
        ax[i-1].set_extent([-125, -109, 40.5, 49.5]) 
        #ax[i-1].set_global()
        
        #ax.stock_img()
        
        ax[i-1].add_feature(cfeature.LAND) 
        ax[i-1].add_feature(cfeature.LAKES)
        ax[i-1].add_feature(cfeature.RIVERS)
        ax[i-1].add_feature(cfeature.OCEAN)
        ax[i-1].add_feature(cfeature.COASTLINE)
        ax[i-1].add_feature(cfeature.STATES)
        ax[i-1].add_feature(cfeature.BORDERS, linestyle=':')
        ax[i-1].coastlines()
    
        unit_list = 'FB (%)'
        ax[i-1].set_title(abc)
...
# =============================================================================
                    ...
                    plotted_figure = ax[i-1].scatter(np.reshape(x,-1), np.reshape(y,-1), marker=marker_shape,c = np.reshape(color,-1), s = size,cmap=cmap, transform=ccrs.Geodetic())#,ax=ax)
