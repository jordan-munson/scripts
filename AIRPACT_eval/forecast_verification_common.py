...
for species in pollutant:
    ...
    if species == 'O3':     
        x = pd.merge(x,df_aqsid_o3,on='AQSID') # isolate to common aqsid
        x = x.set_index('datetime') # have to have set index after the merge
        x = x.resample('H').mean()
        ...
        x = pd.merge(x,df_aqsid_pm,on='AQSID') # isolate to common aqsid
